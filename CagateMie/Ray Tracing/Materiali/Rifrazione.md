...
- **r'⊥** (r perp): la componente parallela alla superficie (perpendicolare alla [[Le Normali|normale]])
- **r'∥** (r par): la componente lungo la [[Le Normali|normale]] (perpendicolare alla superficie)
...
dove v è la direzione del [[Raggio (Ray)]] incidente (unitario) e N è la [[Le Normali|normale]] unitaria. Il termine cos(θ) si ottiene con il [[Vettori 3D|dot product]]: cos(θ) = - (v · N) (il meno perché v punta verso la superficie, mentre N punta nella direzione opposta alla superficie). Quindi:
...
Se questo valore sotto radice diventa negativo, si ha la [[Riflessione Totale Interna]].
